...
- `my_button_basics` How to interface with buttons (switches).
- `my_gpio_basics` How to interact with GPIO pins of the development board.
- `my_gpio_scanner` How to scan all GPIO pins and discover their I/O function.
- `my_i2c_scanner` How to scan all slave devices on the I2C pins, and identify their I2C slave address. This is handy when working with new I2C slave devices.
- `my_ledc_pwm_basics` How to use the standard ESP-IDF LEDC driver (a LED Controller driver using PWM).
- `my_nvs_basics` How to use the standard ESP-IDF NVS (Non-Volatile Storage) driver with a custom NVS partition.
- `my_rmt_basics` How to use the standard ESP-IDF RMT driver.
- `my_spiffs_basics` How to use the standard ESP-IDF SPIFFS file system driver.
- `my_sw180_tilt_sensor` How to interface with this tilt sensor (no extra components needed).
- `my_timer_basics` How to use the standard ESP-IDF Timer driver.
- `my_uart_basics` How to use the standard ESP-IDF UART driver.
...
- `my_am2320_temperature_sensor_using_lib` How to read data from the Aosong AM2320 meteo sensor.
- `my_bh1750fvi_lightsensor_using_lib` How to read data from the BH1750 light intensity sensor.
- `my_bme280_sensor_using_lib` How to read data from the Bosch BME280 meteo sensor.
- `my_bmp280_sensor_using_lib` How to read data from the Bosch BMP280 meteo sensor.
- `my_deep_sleep_wakeup` Demonstrates how to use a switch or a magnetic door/window sensor to wake up an ESP32 from deep sleep. 
- `my_dht11_temperature_sensor_using_lib` How to read data from the Aosong DHT11 temperature sensor.
- `my_dht22_temperature_sensor_using_lib` How to read data from the Aosong DHT22/AM2302 temperature sensor.
- `my_door_sensor_reed_switch` Demonstrates how to use a magnetic door/window sensor which is based on a reed switch.
- `my_ds3231_clock_using_lib` How to get/set data from the DS3231 ZS042 RTC realtime clock board.
- `my_hcsr501_pir_sensor_using_lib` How to read data from the HC-SR501 PIR human infrared sensor.
- `my_huzzah32_battery_voltage_using_lib` How to use specific features of the Adafruit HUZZAH32 development board. example: read battery voltage level. 
- `my_ky032_obstacle_sensor_using_lib` How to read data from the KY-032 infrared obstacle avoidance sensor.
- `my_ledrgb_using_lib` How to control RGB LED strips (such as the Adafruit Neopixels and BTF-LIGHTNING products).
- `my_linked_list_basics` How to use the Linked List component.
- `my_neom8n_gps_using_lib` How to get GPS data from the GPS Ublox NEO-M8N module.
- `my_wifi_device_scanner` How to scan all Wifi channels and discover the devices.
- `my_wifi_ssid_cloner` How to clone existing Access Points.
- `my_wifi_ssid_scanner` How to scan all Wifi channels and discover the Access Points.
- `my_wifi_ssid_spammer` How to create additional Access Points in the area.
